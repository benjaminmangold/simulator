...
# -----------------------------------------------------------------------------
# Pages with weights for realistic variance in view distribution
# Higher weight = more likely to be viewed
# Weights are randomised per run to create variance across days
# -----------------------------------------------------------------------------
def build_page_weights() -> List[Dict]:
    """
    Build page list with randomised weights.
    Called once per run to create natural variance in page view distribution.
    """
    # Base weights with variance ranges (min, max)
    # This creates different "popular" pages each run
    page_configs = [
        {"path": "/", "weight_range": (15, 35)},  # Homepage: high traffic but variable
        {"path": "/blog/", "weight_range": (8, 20)},
        {"path": "/blog/google-analytics-4-course/", "weight_range": (5, 18)},
        {"path": "/blog/ga4-events/", "weight_range": (4, 15)},
        {"path": "/training/", "weight_range": (6, 16)},
        {"path": "/courses/", "weight_range": (8, 22)},
        {"path": "/contact/", "weight_range": (2, 8)},
        {"path": "/about/", "weight_range": (2, 10)},
        # Thank you page: 1-2% of traffic (low weight relative to total)
        {"path": "/signup/thank-you", "weight_range": (1, 3)},
    ]
    
    pages = []
    for config in page_configs:
        weight = random.randint(config["weight_range"][0], config["weight_range"][1])
        pages.append({"path": config["path"], "weight": weight})
    
    return pages

# Build weights once per run
PAGES_WEIGHTED = build_page_weights()

# Simple list for backward compatibility where needed
PAGES = [p["path"] for p in PAGES_WEIGHTED]

def weighted_page_choice() -> str:
    """Select a page based on weights."""
    total = sum(p["weight"] for p in PAGES_WEIGHTED)
    r = random.uniform(0, total)
    upto = 0.0
    for p in PAGES_WEIGHTED:
        if upto + p["weight"] >= r:
            return p["path"]
        upto += p["weight"]
    return PAGES_WEIGHTED[-1]["path"]

def select_session_pages(count: int) -> List[str]:
    """
    Select pages for a session using weighted distribution.
    Ensures no immediate duplicates (same page twice in a row).
    """
    pages = []
    for _ in range(count):
        attempts = 0
        while attempts < 10:
            page = weighted_page_choice()
            # Avoid immediate duplicate (same page twice in a row)
            if not pages or page != pages[-1]:
                pages.append(page)
                break
            attempts += 1
        else:
            # Fallback: just pick any different page
            available = [p["path"] for p in PAGES_WEIGHTED if not pages or p["path"] != pages[-1]]
            pages.append(random.choice(available) if available else weighted_page_choice())
    return pages

# -----------------------------------------------------------------------------
# Products and ecommerce page paths
# -----------------------------------------------------------------------------
PRODUCTS = [
    {"id": "sku_201", "name": "GA4 Complete Course", "category": "Courses", "price": 225.00, "page_path": "/courses/ga4/"},
    {"id": "sku_202", "name": "Google Tag Manager Course", "category": "Courses", "price": 225.00, "page_path": "/courses/gtm/"},
    {"id": "sku_203", "name": "Looker Studio Course", "category": "Courses", "price": 125.00, "page_path": "/courses/looker-studio/"},
    {"id": "sku_204", "name": "Google Ads Fundamentals Course", "category": "Courses", "price": 225.00, "page_path": "/courses/google-ads/"},
]

# Checkout page path for begin_checkout and purchase events
CHECKOUT_PAGE_PATH = "/checkout/"
THANK_YOU_PAGE_PATH = "/checkout/thank-you/"
...
def event_payload(client_id: str, event: Dict, timestamp_micros: int, user_properties: Optional[Dict]) -> Dict:
    payload: Dict = {
        ...
        # Consent Mode v2 signals - all granted
        "consent": {
...
def path_to_title(path: str) -> str:
    """Convert a path to a page title."""
    clean = path.strip("/")
    if not clean:
        return "Home"
    
    # Acronyms/terms that should preserve specific casing
    case_overrides = {
        "ga4": "GA4",
        "gtm": "GTM",
        "seo": "SEO",
    ...
    def format_part(part: str) -> str:
        ...
        for word in words:
            lower = word.lower()
            if lower in case_overrides:
                formatted.append(case_overrides[lower])
            else:
    ...
    parts = clean.split("/")
    return " - ".join(format_part(part) for part in parts)
...
def simulate_one_session(client_id: str) -> None:
    ...
    # Choose pages for this session using weighted distribution
    page_count = random.randint(2, 6)
    paths = select_session_pages(page_count)
    ...
    pv1_params = {
        ...
        "page_title": path_to_title(first_path),
        "language": lang,
    ...
    if random.random() < 0.25:
        ...
        product_page_path = product.get("page_path", "/courses/")
        ...
        def send_ecom(name: str, extra_params: Optional[Dict], offset_ms: int, page_path: str):
            """Send ecommerce event with page context."""
            ...
            params = {
                ...
                # Associate ecommerce events with page context
            ...
            print_validation(data)
            print(f"[{client_id[:10]}...] Sent: {name} on {page_path} | Status: {status}")

        # view_item happens on the product page
        send_ecom("view_item", {"value": price}, t_cursor + 4_000, product_page_path)

        if random.random() < 0.60:
            # add_to_cart also happens on the product page
            send_ecom("add_to_cart", {"value": price}, t_cursor + 8_000, product_page_path)

            if random.random() < 0.55:
                # begin_checkout happens on the checkout page
                send_ecom("begin_checkout", {"value": price}, t_cursor + 13_000, CHECKOUT_PAGE_PATH)

                if random.random() < 0.35:
                    # purchase happens on the thank you page
                    send_ecom(
                        "purchase",
                        {
...
def main() -> None:
    ...
    # Log page weights for this run (helps debug variance)
    print("Page weights this run:")
    for p in PAGES_WEIGHTED:
        print(f"  {p['path']}: {p['weight']}")
